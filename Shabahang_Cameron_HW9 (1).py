...
def candlestick_plot(ticker, short=5, long=15):
    ...
    for i in range(len(data)):
        if rawdata['Open'][i]>rawdata['Close'][i]:
            #If intraday movement is negative
            #Record total positive difference and subtract intraday movement to reset high
            #Record total negative difference as low
            high.append(rawdata['High'][i]-rawdata['Open'][i]-data[i])
            low.append(rawdata['Close'][i]-rawdata['Low'][i])
        else:
            #Else if intraday movement is positive
            #Record total positive difference as high
            #Record total negative difference and add intraday movement to reset low
    ...
    #Quantifies the height of the candlestick
    extrema = [low,high]
    #print(extrema)

    #Implementation of candlestick plot using matplotlib
    plt.figure(figsize=(10,6))
    #Takes the parameter extrema
